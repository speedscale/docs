...
Public prose follows `/Users/kahrens/spd-workspace/spd-ai-notes/INSTRUCTION - Read Before Writing Content.md`. Run `yarn check:writing` before pushing. The command checks added lines by default, `--base <sha>` selects a comparison base, and `--all` audits all tracked public-writing files.
