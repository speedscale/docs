...
The Speedscale model can be trained using the [store_sig](../transformation/transforms/store_sig.md) transform. This transform will add the specified data to the signature and use it to match uniqueness. This transform allows you to add new items to check before accepting a service mock request match.
