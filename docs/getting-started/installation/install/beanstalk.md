...
We're also configuring our app to use `goproxy` as an outbound http proxy so that we can capture outbound requests. Our example `notifications` app uses Golang so the variables are set accordingly in the manifest. The environment variables depend on the language of your app so refer to [proxy server configuration](/getting-started/installation/sidecar/proxy-modes.md#configuring-your-application-proxy-server)
and [trusting TLS certificates](/getting-started/installation/sidecar/tls/#trusting-tls-certificates).
