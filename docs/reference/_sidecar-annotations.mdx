...
| `sidecar.speedscale.com/proxy-protocol`      | Set the protocol for the outbound proxy server. Supported values are `tcp+socks`, or `tcp+http`. Only valid if `capture-mode` is `proxy` and `proxy-type` is not `transparent`, ignored otherwise. |
