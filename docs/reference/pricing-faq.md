...
Apply [traffic filters](https://docs.speedscale.com/configuration/filters) to only capture data from specific endpoints, headers, or service patterns. This reduces noise and focuses ingest on meaningful interactions. The most common "noisy" traffic patterns include heartbeats, monitoring systems and database keepalives. Common systems like Datadog and New Relic are filtered out in the default `standard` filter set (for example). Learn more about configuring your own filters [here](/guides/creating-filters/). Sometimes it's easier to simply block an entire set of ports or hosts at the sidecar level. To do that, check out the [ignore-src-ips](https://docs.speedscale.com/setup/sidecar/annotations/#sidecarspeedscalecomignore-src-ips) and [ignore-src-hosts](https://docs.speedscale.com/setup/sidecar/annotations/#sidecarspeedscalecomignore-src-hosts) annotations.
