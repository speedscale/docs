...
| `test.speedscale.com/cleanup`  | **renamed** |`replay.speedscale.com/cleanup`. Additionally, *possible values are changed* and this is *no longer boolean*. New values are: `inventory`, `all` or `none`. Defaults to `inventory` if annotation is not present. `inventory` instructs the operator to clean up all objects relate dto Speedscale's replay of traffic once finished (such as ConfigMaps, traffic generators, collectors, and responders, and related infrastructure), as well as reverting changes made to the workload. `all` instructs the operator to clean up all objects as well as the workload under test (such as a Deployment, Job, DaemonSet, or StatefulSet). `none` instructs the operator to not do anything with replay resources once a replay is finished. |
...
| `replay.speedscale.com/timeout` | **new** | Specifies a timeout for a replay. Ignored with replay mode is `responder-only` |
