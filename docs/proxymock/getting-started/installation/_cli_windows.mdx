The `proxymock` cli is available for and can run on Windows, but *does not* support native traffic capture. These instructions are included in case you know what you are doing and want to install `proxymock` manually.
