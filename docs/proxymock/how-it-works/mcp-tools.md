...
| `test_config` | string | no | A test config authored in this workspace (proxymock/testconfigs/&lt;name&gt;.json; create and edit one with the test_config tool), or a path to a config JSON file, or the name of a built-in Speedscale config (regression, regression_no_mocks, standard, the performance and chaos configs). It is the base config this replay runs: load shape, chaos and generator behaviour come from it, and the replay is held to its goals and assertion groups. Precedence: the parameters on this tool win over the named config, which wins over the built-in regression config used when this is omitted. Fields a local replay does not honour are reported rather than silently dropped. |
| `tests-filter` | string | no | Filter choosing which recorded RRPairs to replay as tests; everything else is mocked. Default replays inbound traffic. To load a database the app called, use '(direction IS OUT) AND (tech IS Postgres)' with test-against set to the database address. Keep inbound tests too with '(direction IS IN) OR (tech IS Postgres)'. Each parenthesised group holds one filter type. |
...
- 'defaults' (read-only): where the traffic was recorded and where to replay it by default. With 'snapshot_id' (or 'report_id') it returns what the dashboard's replay wizard pre-fills: the cluster, namespace, per-inbound-slice workload, mocks and test config, from the latest earlier replay of the snapshot, else the namespace it was captured from, and says which evidence it used. With 'in_directories' it reads a recording on disk instead (no login needed): captured in a cluster (Kubernetes tags on the inbound RRPairs, or a pulled snapshot's metadata) or recorded locally by proxymock, plus the address the busiest inbound slice was recorded against, which for a local recording is where the app listened.

Run:
- 'start' (mutates): push the recordings in 'in_directories' (or reuse 'snapshot_id'), then start the replay in 'cluster' and 'namespace'. Give it a destination: 'workload' (every inbound slice goes to that workload), 'routes' (send individual slices to their own workload, in any namespace, or address; combine with 'workload' for the rest), or 'target' (replay against an address; nothing in the cluster is modified and nothing is mocked). Set 'dry_run' to resolve the cluster, routes, mocks and test config and report them without pushing or starting anything, like 'proxymock cloud replay --dry-run'. Omitting both 'mocks' and 'mock_enabled' mocks every recorded outbound dependency of the workload, which is what makes a replay repeatable - the default of every workload replay path: Speedscale cloud, 'speedctl infra replay', 'proxymock cloud replay', 'proxymock cluster replay start' and the proxymock web Replay tab. List keys in 'mocks' to mock only those, or set 'mock_enabled' to false to mock nothing so the workload reaches its real dependencies. Mocking needs a workload to attach a responder to, so a 'target' replay mocks nothing, and asking one to mock is an error. The keys 'mocks' takes come from the cluster tool's action='replay-prepare'. A parameter that does not apply to the replay shape you picked (for example 'snapshot_name' with 'snapshot_id') is refused rather than ignored. Returns as soon as Speedscale cloud accepts the replay, with the report id and dashboard URL. It does NOT wait for the replay to finish.
...
- 'status' (read-only): one report's status as Speedscale cloud sees it (Initializing, Testing, Analyzing, then a terminal verdict: Passed, Missed Goals, Error or Canceled), whether it is done, its success rate, the replay events its components raised (oldest first, each with severity, reason, description and suggested resolutions; they explain a stuck, failed or skewed replay) and, once analyzed, its goals with expected and actual values. Pass the 'report_id' that 'start' returned. Set 'wait' to block until the verdict (bounded by 'timeout_seconds') and get the status changes and new events seen along the way; otherwise call it again a few seconds apart until done is true. Then pull_report downloads the report and its snapshot for analysis.

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `action` | string | **yes** | Which operation to run. Read-only: 'clusters', 'namespaces', 'workloads', 'defaults', 'status'. Mutating: 'start', 'cancel'. |
...
| `in_directories` | array | no | action=defaults: the recording to read the origin of. action=start: directories holding the RRPair files to push as the snapshot. Defaults to the working directory. With 'snapshot_id' nothing is pushed, so it is refused there unless it locates the workspace a 'test_config' name is read from. |
| `inspector_id` | string | no | action=namespaces, workloads and start: the cluster to use, by its inspector id. Pass this or 'cluster'. |
| `mock_attach` | array | no | action=start: attach the mocks for the outbound dependencies a selector matches to a workload other than the default, each entry as SELECTOR=[NAMESPACE/]KIND/NAME (for example 'svc:postgres=deployment/banking-db' or 'glob:mysql*=statefulset/orders-db'). Selectors: exact key, svc:&lt;substring&gt;, host:&lt;hostname&gt;, glob:&lt;pattern&gt;; keys come from the cluster tool's action='replay-prepare', and a selector that matches no mocked dependency is refused. The last matching entry wins; everything unmatched attaches to 'mock_attach_default', else to the first non-Service workload route. KIND is deployment, statefulset, daemonset, replicaset or rollout: a Service cannot carry a responder, so name the workload behind it. Works with the default mock-everything and with 'mocks'; refused when mocking is off (mock_enabled=false or no_mocks). The same vocabulary as 'speedctl infra replay --mock-attach'. |
| `mock_attach_default` | string | no | action=start: attach the mocks no 'mock_attach' selector matches to this workload, as [NAMESPACE/]KIND/NAME, instead of the first non-Service workload route. Use it when the system under test is a Service, or its dependencies are mocked beside another workload. Not a Service. Refused when mocking is off. |
| `mock_enabled` | boolean | no | action=start: true or omitted mocks every recorded outbound dependency of the workload (narrow it with 'mocks'); false mocks nothing. The same switch as the proxymock web Replay tab's 'Mock dependencies' checkbox and the cluster tool's 'mock_enabled'. |
| `mocks` | array | no | action=start: outbound dependency keys to mock, taken verbatim from the cluster tool's action='replay-prepare'. Only applies to a workload replay. Omitting both 'mocks' and 'mock_enabled' mocks every recorded outbound dependency of the workload, which is what makes a replay repeatable - the default of every workload replay path: Speedscale cloud, 'speedctl infra replay', 'proxymock cloud replay', 'proxymock cluster replay start' and the proxymock web Replay tab. List keys in 'mocks' to mock only those, or set 'mock_enabled' to false to mock nothing so the workload reaches its real dependencies. Mocking needs a workload to attach a responder to, so a 'target' replay mocks nothing, and asking one to mock is an error. |
...
| `no_mocks` | boolean | no | action=start: mock nothing, the same as mock_enabled=false; kept for existing callers. Refused together with 'mocks' or mock_enabled=true. |
| `report_id` | string | no | action=status and cancel: the report id action='start' returned. |
| `routes` | array | no | action=start: send one inbound slice to its own destination, each entry as SLICE=WORKLOAD, SLICE=NAMESPACE/WORKLOAD, SLICE=NAMESPACE/KIND/WORKLOAD or SLICE=scheme://host:port (an address). Slice keys come from the cluster tool's action='replay-prepare'; each slice may be routed once. Combine with 'workload', which takes every slice not routed here; mutually exclusive with 'target'. The same vocabulary as the cluster tool's 'routes' and the proxymock web Replay tab. Mocks attach to the first non-Service workload route unless 'mock_attach' or 'mock_attach_default' place them elsewhere. |
| `snapshot_id` | string | no | action=start: replay a snapshot already in Speedscale cloud instead of pushing the local recordings. action=defaults: the snapshot to find the recording origin and default replay target of. |
...
| `test_config` | string | no | action=start: a test config authored in this workspace (proxymock/testconfigs/&lt;name&gt;.json; create and edit one with the test_config tool), or a path to a config JSON file. It is pushed to Speedscale cloud under the same name, then named by id on the replay, so there is no separate push step. The name of any built-in Speedscale config (regression, regression_no_mocks, standard, the performance and chaos configs) is named as-is, as is the id of a test config already in the tenant (for example one created in the dashboard) when no workspace config has that name; neither is pushed. Omit for the built-in regression config. |
| `timeout_seconds` | number | no | action=status with wait=true: how long to wait, in seconds (default 240, at most 600). If the replay is still running when it runs out, call again. |
| `wait` | boolean | no | action=status: poll the report every 5s until it reaches a terminal status (Passed, Missed Goals, Error, Canceled) or 'timeout_seconds' runs out, and list the status changes and new replay events seen along the way. Only reads the report; running out of time never stops the replay. |
...
Score one replay, cloud or local, in one JSON shape. Read-only: nothing is downloaded or written. Call it after every tuning iteration to see whether the snapshot got better.

Returns:
- accuracy: replayed responses that matched the recorded ones (pairs, matched, statusMismatches, bodyMismatches, rate, topFailingEndpoints). Cloud reports also carry the report's own reportSuccessRate.
- matchRate: outbound calls the mocks answered, from the responder status of each call (HIT, MISS, PASSTHROUGH): total, matched, noMatch, passthrough, rate, noMatchRate, passthroughRate, topMissingSignatures, topMissingHosts. Local replays also carry matchRate.analysis, the offline match-rate analysis of the same run with its projected rate under the active tuning blueprints; its reportRate counts passthrough as matched, so it differs from matchRate.rate.
- goals: the goal verdict and each goal's condition and observed value.
- sources: the file each section was read from. notes: why a section is null.

'input' is one of: a cloud report id already pulled into the workspace (pull it with pull_report first; this tool never downloads), a pulled workspace (its single report), a pulled report's report-&lt;id&gt; run directory, or a local replay output directory holding replay-verdict.json (written by replay_traffic). A local replay's match rate comes from the mocked-* run (the mock server output) that served it: the run whose RRPairs overlap the replay in time, else the latest one started before the replay, else the newest one with a note that the pairing is a guess. Pass 'mock_run' to name it. matchRate.mockRun and mockRunSelection say which run was used and how. Its analysis matches the mocks tool's analyze action.

Pass 'baseline' (another input of any kind) to include its score and per-number deltas, e.g. this iteration against the previous one, or a local run against the cloud run it is tuning toward.

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `input` | string | **yes** | A pulled cloud report id, a pulled workspace, a report-&lt;id&gt; run directory, or a local replay output directory holding replay-verdict.json. |
| `baseline` | string | no | Optional second input of any of the same kinds. When set, the result includes the baseline's score and deltas (this score minus the baseline's). |
| `mock_run` | string | no | Optional mocked-* run directory that served a local input replay, overriding the time-overlap pairing. A path, or a run name inside the workspace. Applies to the input, not the baseline. |
| `workspace` | string | no | Directory a bare report id is looked up in: the repo root or its proxymock directory. Defaults to the server's working directory. |
...
Other actions: 'list' (workspace configs plus every read-only built-in Speedscale config, 'regression' first), 'show' (one config with warnings and validation problems), 'validate' (check a saved config by 'id' or an unsaved 'config' document without writing), 'delete' (remove a workspace config; requires confirm=true).
...
| `from` | string | no | create: the config to copy, a workspace config id or any built-in Speedscale config name ('regression' is the default). Cannot be combined with 'config'. |
...
| `mock_enabled` | boolean | no | action=replay-start: true or omitted mocks every recorded outbound dependency of the workload (narrow it with 'mocks'); false mocks nothing. The same switch as the proxymock web Replay tab's 'Mock dependencies' checkbox and the cloud_replay tool's 'mock_enabled'. |
| `mocks` | array | no | action=replay-start: outbound dependency keys to mock, taken verbatim from 'replay-prepare'. Only applies to a workload replay, because mocking needs a responder. Omitting both 'mocks' and 'mock_enabled' mocks every recorded outbound dependency of the workload, which is what makes a replay repeatable - the default of every workload replay path: Speedscale cloud, 'speedctl infra replay', 'proxymock cloud replay', 'proxymock cluster replay start' and the proxymock web Replay tab. List keys in 'mocks' to mock only those, or set 'mock_enabled' to false to mock nothing so the workload reaches its real dependencies. Mocking needs a workload to attach a responder to, so a 'target' replay mocks nothing, and asking one to mock is an error. |
...
| `test_config` | string | no | action=replay-start: a test config authored in this workspace (proxymock/testconfigs/&lt;name&gt;.json; create and edit one with the test_config tool), or a path to a config JSON file. It is compiled and staged in the in-cluster forwarder alongside the snapshot, so the replay runs it without any Speedscale cloud round-trip. This is how responder replicas and resources get set on a cloud-free replay. Fields the in-cluster path does not honour are reported rather than silently dropped. The name of any built-in Speedscale config (regression, regression_no_mocks, standard, the performance and chaos configs; maintained by Speedscale, read-only) is staged the same way, and omitting this is the built-in regression config. A test config that exists only in Speedscale cloud has to be pulled into the workspace first (proxymock cloud pull test-config &lt;id&gt;). A workspace config cannot be combined with 'snapshot_id', which skips the staging this rides on. With namespaced=true a built-in is named by id and read from Speedscale by the replay itself, and a workspace config travels compiled inside the replay request (the install's replay coordinator has to support it, or the request is refused before anything is written). |
