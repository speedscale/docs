import React from 'react';
import Card from '@site/src/components/Card';
import CardHeader from '@site/src/components/Card/CardHeader';
import CardBody from '@site/src/components/Card/CardBody';
import CardFooter from '@site/src/components/Card/CardFooter';

const BrowserCaptureCard = () => {
  return (
    <Card shadow="md">
      <CardHeader 
        style={{ backgroundColor: '#f8f9fa', borderBottom: '1px solid #e0e0e0' }}
        textAlign="center"
        weight="Bold"
      >
        <h3 style={{ margin: 0, marginBottom: '1rem' }}>🌐 Browser Testing</h3>
      </CardHeader>
      <CardBody className="padding-vert--md">
        Capture and replay browser-based API traffic. Perfect for testing 
        web applications and frontend API interactions.
      </CardBody>
      <CardFooter style={{ backgroundColor: '#f8f9fa', textAlign: 'center' }}>
        <a href="./browser" className="button button--primary">Test Browsers</a>
      </CardFooter>
    </Card>
  );
};

export default BrowserCaptureCard;